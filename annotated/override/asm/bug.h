...
do {								\
	...
	/* FRAGMA: ud2 asm elided.  WP models volatile asm as havoc-all,
	 * which can never satisfy an assigns clause, even on this
	 * never-returning path.  The trap semantics are carried by
	 * __builtin_unreachable -> fragma_unreachable (noreturn,
	 * ensures \false; see specs.h). */			\
