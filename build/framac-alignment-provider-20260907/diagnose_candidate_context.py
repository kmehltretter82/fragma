#!/usr/bin/env python3
"""Run all 55 unchanged Hexagon compiler/private-analyzer context pairs.
...
def snapshot(rt, c, h, old, classifiers_pin, sources):
    extra = {**PINS, BASE / "context_classifiers.py": classifiers_pin}
    ...
    return {"selected": selected, "compiler_link": h.record(Path(COMPILER)),
            "private": private, "runtime_artifacts": runtime_tree, "historical_artifacts": historical_tree,
...
def main():
    ...
    classify = load(BASE / "context_classifiers.py", args.classifiers_sha256)
