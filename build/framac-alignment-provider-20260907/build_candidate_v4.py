...
Validate the original archive plus six ordered explicit reviewed patch hashes against
...
SOURCE = BASE / "worktree-4/source/frama-c-33.0-Arsenic"
PREPARATION = BASE / "worktree-4/preparation.json"
PREPARATION_SHA = "964fa818a6d963513fbe7acce0df00d81f51150415add0c1ebbc831620eadd79"
...
def validate_source(h, patch_pins):
    ...
    for name, allowed in PATCH_FILES.items():
        ...
        # 0004 through 0007 share cabs2cil.ml; coordinates/context remain strict.
    ...
    require(preparation.get("prepared_source_entries") == expected,
            "Preparation final inventory differs from ordered six-patch reconstruction")
...
def main():
    ...
    patch_pins = dict(zip(PATCH_FILES, (getattr(args, f"patch_{n}_sha256") for n in (2, 3, 4, 5, 6, 7))))
    require(all(re.fullmatch(r"[0-9a-f]{64}", value) for value in patch_pins.values()), "Need six explicit SHA-256 pins")
    ...
    require(BASE == ROOT / "build/framac-alignment-provider-20260907" and output.parent == BASE
            and output.name == "candidate-build-4"
            and not os.path.lexists(output), "Need fresh fixed-scope candidate-build-4 output")
    ...
    try:
        ...
        h.save(output / "source-validation.json", {"status": "exact-original-plus-six-ordered-patches",
