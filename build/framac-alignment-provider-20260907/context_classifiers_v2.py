"""Pure version-2 observations for the unchanged 55 private context pairs.
...
def _locals_analyzer_values(old, printed, case):
    """Parse only the four observed hoisted-local declaration/body shapes.

    The frozen parser already allowed prefixed witness identifiers, but did
    not recognize the private printer's multiline __used__ attribute. Do not
    broaden that parser globally: require the exact local symbol and complete
    fixed uint64/global/function shape here. Values are observations, not
    substituted expectations; original alignment requests must remain present.
    """
    _check_case(case)
    fields = [
        "c11_local_size", "c11_expression_alignment", "c11_type_alignment",
        "gnu_local_size", "gnu_expression_alignment", "gnu_type_alignment",
    ]
    _require(case["fixture"] == "locals" and case["fields"] == fields,
             "Hoisted-local parser requires the exact locals witness inventory")
    symbol = "fragma_scope_fragma_alignment_witness"
    prefix = (
        "/* Generated by Frama-C */\n"
        "void fragma_scope(void);\n\n"
        "static unsigned long long const " + symbol
        + "[6] __attribute__((\n  __used__)) = {"
    )
    requested = str(case["alignment"])
    suffix = (
        "};\n"
        "void fragma_scope(void)\n"
        "{\n"
        "  int _Alignas(" + requested + ") fragma_c11_local;\n"
        "  int fragma_gnu_local __attribute__((__aligned__(" + requested + ")));\n"
        "  return;\n"
        "}\n\n\n"
    )
    match = old.re.fullmatch(
        old.re.escape(prefix) + r"([^{}\n]+)" + old.re.escape(suffix), printed)
    _require(match is not None,
             "Unexpected complete hoisted-local uint64 witness/function/global shape")
    tokens = match[1].split(", ")
    _require(len(tokens) == 6
             and all(old.re.fullmatch(r"(?:0|[1-9][0-9]*)ULL", token) for token in tokens),
             "Hoisted-local witness did not contain exactly six uint64 constants")
    values = [int(token[:-3]) for token in tokens]
    _require(all(value < 2 ** 64 for value in values), "Hoisted-local value outside uint64")
    return {"values": dict(zip(fields, values)), "witness_symbol": symbol}
...
def analyzer_observation(old, code, stdout, stderr, printed, case):
    ...
    try:
        ...
        if code == 0:
            ...
            values = (_locals_analyzer_values(old, printed, case)
                      if case["fixture"] == "locals"
                      else old.analyzer_values(printed, case["fields"]))
            return {"status": "constant-witness-observed", **values,
                    "primary_diagnostics": [], "diagnostic_blocks": []}
