#!/usr/bin/env python3
"""Prepare only fresh worktree-5 from the pinned archive and seven ordered patches.

No subprocess, build, compiler, analyzer, package or installation is invoked.
The frozen V4 builder supplies only authenticated read/hash/archive/diff helpers.
...
BUILDER = BASE / "build_candidate_v4.py"
BUILDER_SHA = "137a711930a309399a220c5315ae298da7f950fdc57bcfb99b69dd577a728084"
OUTPUT = BASE / "worktree-5"
...
PATCH_PINS = {
    ...
    "0008-vla-bound-local-init.patch": "96d2d3a68fa0c4172a498ed7f9ffa96fdb6152da32d737e427a4ee6bbd2d46d5",
}
ORIGINAL_PROPOSALS = {
...
def load_builder():
    ...
    module = types.ModuleType("fragma_v5_preparation_frozen_builder")
...
def reconstruct(b, h):
    ...
    # The frozen V4 archive helper materializes all ten edited source files.
    for name in ("source-before.json", "source-after.json"):
    ...
    patch_files = {
        **b.PATCH_FILES,
        "0008-vla-bound-local-init.patch": {"src/kernel_internals/typing/cabs2cil.ml"},
    }
    ...
    b.require(len(current) == 10 and len(changes) == 7
              and sum(len(f["hunks"]) for p in changes for f in p["files"]) == 63,
              "Expected ten source files and seven patches/63 hunks")
...
def main():
    ...
    b.require(BASE == ROOT / "build/framac-alignment-provider-20260907"
              ...
              and not os.path.lexists(output), "Need unused, exact worktree-5 output")
    ...
    try:
        ...
        report.update(original_source_entries=original, patch_changes=changes,
                      changed_file_count=len(final_bytes), hunk_count=63)
        ...
        if expected is not None:
            try:
                ...
                b.require(actual == expected, "Prepared source differs from seven ordered patches")
            except (OSError, ValueError, KeyboardInterrupt) as exc:
