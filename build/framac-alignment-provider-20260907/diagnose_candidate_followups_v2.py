...
The complete first followup tree remains pinned raw history, not semantic approval.
"""
...
RECORDER = BASE / "diagnose_candidate_context_v5.py"
RECORDER_SHA = "4ad830dd16cf12323f52806cd74bc76e462e52f02f6203599e0ede0f61f70416"
...
PREVIOUS_FOLLOWUP = BASE / "followup-run-1"
PREVIOUS_FOLLOWUP_SHA = "14aaeb2d172ab7ea98661984ea512f53affeedf0eed73b91ac9820f3f692f2ff"
PREVIOUS_RECORDER = BASE / "diagnose_candidate_followups.py"
PREVIOUS_RECORDER_SHA = "89bbbb5a41a8b1ee281dd8dc602019fc9ef9d70c9244c3d1b9858c5db0c9ba15"
...
def snapshot(d, rt, c, h, old, specifications, sources, pins):
    ...
    previous, previous_artifacts = d.retained_tree(c, h, PREVIOUS_FOLLOWUP, PREVIOUS_FOLLOWUP_SHA)
    require(previous["status"] == SUCCESS and previous["input_drift"] is False
            and previous["cancelled"] is False and previous["query_limit"] == QUERY_LIMIT
            and previous["planned_case_count"] == 44 and len(previous["commands"]) == QUERY_LIMIT
            and previous["executed_query_count"] == 150 and previous["skipped_query_count"] == 72
            and sum(row["child_started"] is True for row in previous["commands"]) == 150
            and sum(row["execution"] == "skipped" for row in previous["commands"]) == 72
            and previous["preprocessing_closure_complete"] is True
            and previous["printed_source_checks_complete"] is True,
            "Previous followup raw inventory changed; it is not semantic approval")
    selected = {str(path): d.record(h, path)
                for path in (Path(__file__).resolve(), RECORDER, ARITHMETIC, VLA, PREVIOUS_RECORDER)}
    ...
    return {"context_prerequisites": original, "selected_followups": selected,
            "previous_followup_artifacts": previous_artifacts,
            "previous_followup_scope": "retained raw history only; not semantic approval",
...
def main():
    ...
    pins = {RECORDER: RECORDER_SHA, ARITHMETIC: args.arithmetic_fixtures_sha256,
            VLA: args.vla_fixtures_sha256, PREVIOUS_RECORDER: PREVIOUS_RECORDER_SHA}
    ...
    rt_path, c_path = BASE / "check_candidate_runtime_v5.py", BASE / "build_candidate_v5.py"
    require(d.BUILD == BASE / "candidate-build-5" and d.RUNTIME == BASE / "candidate-runtime-5",
            "Context reader is not the fixed fifth private candidate")
