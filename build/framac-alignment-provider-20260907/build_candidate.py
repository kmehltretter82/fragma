...
Validate the original archive plus three explicit reviewed patch hashes against
...
SOURCE = BASE / "worktree-1/source/frama-c-33.0-Arsenic"
PREPARATION = BASE / "worktree-1/preparation.json"
PREPARATION_SHA = "f66072457de94d983a26964e3db422eac72d812912a0cff06c0367963bc4ea1d"
...
PATCH_FILES = {
    "0002-alignment-attribute-policy.patch": {
...
def validate_source(h, patch_pins):
    preparation_data, _ = checked_bytes(h, PREPARATION, PREPARATION_SHA)
    ...
    require(preparation.get("schema_version") == 1
            ...
            and preparation.get("integration_eligible") is False, "Preparation identity differs")
    ...
    for name, allowed in PATCH_FILES.items():
        ...
        patched, changes = patched_bytes(data.decode("utf-8"), originals, allowed)
...
def main():
    ...
    patch_pins = dict(zip(PATCH_FILES, (getattr(args, f"patch_{n}_sha256") for n in (2, 3, 4))))
    require(all(re.fullmatch(r"[0-9a-f]{64}", value) for value in patch_pins.values()), "Need three explicit SHA-256 pins")
    h = load_helper()
    ...
    require(BASE == ROOT / "build/framac-alignment-provider-20260907" and output.parent == BASE
            and re.fullmatch(r"candidate-build-[1-9][0-9]*", output.name)
            and not os.path.lexists(output), "Need fresh fixed-scope candidate-build-N output")
    ...
    try:
        ...
        h.save(output / "source-validation.json", {"status": "exact-original-plus-three-patches",
            "changed_file_count": 9, "source_entry_count": len(source_before), "patches": diffs})
