...
Validate the original archive plus four ordered explicit reviewed patch hashes against
...
SOURCE = BASE / "worktree-3/source/frama-c-33.0-Arsenic"
PREPARATION = BASE / "worktree-3/preparation.json"
PREPARATION_SHA = "c9b4a378958dd8200f16ed2bd1dd37ee392497e91fe93161868bdf341ad094de"
...
def validate_source(h, patch_pins):
    ...
    for name, allowed in PATCH_FILES.items():
        ...
        # 0005 after 0004 on cabs2cil.ml; coordinates/context remain strict.
    ...
    require(preparation.get("prepared_source_entries") == expected,
            "Preparation final inventory differs from ordered four-patch reconstruction")
...
def main():
    ...
    patch_pins = dict(zip(PATCH_FILES, (getattr(args, f"patch_{n}_sha256") for n in (2, 3, 4, 5))))
    require(all(re.fullmatch(r"[0-9a-f]{64}", value) for value in patch_pins.values()), "Need four explicit SHA-256 pins")
    ...
    require(BASE == ROOT / "build/framac-alignment-provider-20260907" and output.parent == BASE
            and output.name == "candidate-build-3"
            and not os.path.lexists(output), "Need fresh fixed-scope candidate-build-3 output")
    ...
    try:
        ...
        h.save(output / "source-validation.json", {"status": "exact-original-plus-four-ordered-patches",
