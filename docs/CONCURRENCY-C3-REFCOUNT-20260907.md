...
Status: accepted for one narrow production lifetime decision with eleven
...
[`results/concurrency-c3-ipc-refcount-20260907-13`](../results/concurrency-c3-ipc-refcount-20260907-13/SUMMARY.md).
It passes all 1036 gates, accepts two separately bounded source-linked kernel
properties, and accepts eleven lifetime plus three progress implementation mappings.
The receipt pins 91 input identities and retains 395 raw artifacts. It found no
...
Run `-12` adds the eleventh, little-endian MIPS32r2 mapping and passes 1036/1036,
but its generated explanatory paragraph still said ten profiles. Run `-13`
derives that paragraph's count from the actual inventory, adds a regression
test for the renderer and reruns the complete gate. Independent readback matches
all 91 inputs and 395 retained artifacts. Run `-12` is retained but superseded;
its computed profile table and gates were green, while its prose was stale.
...
by the same build gate. The other eight lifetime profiles use
...
- the instrumented atomic API, generic fallback, LKMM RMW atomicity axiom, and
  x86-64, arm64, RISC-V, s390, ARM32, PowerPC32, SuperH, Alpha, LoongArch and
  MIPS atomic/cmpxchg/barrier implementation files, the pinned LoongArch/MIPS
  Clang target selections, plus UML's SMP Kconfig,
...
Eleven dedicated SMP builds compile the real `ipc/util.o`, not a project wrapper.
Nine use GCC 15.2.0 and GNU binutils 2.46 already present on the machine. The
other two use explicit Clang/LLVM 21.1.8 LoongArch and MIPS target routes; they
do not relabel or activate separately planned general GCC/Frama-C profiles.
...
| MIPS32r2 Malta | 32-bit LE O32, machine 8 | `a45ad683e9c792341dbd8ae53afbfcd0ac919bcbaa3f34e988ff0b04fc3f7278` | `bfc8d8134c04b6b892deb8ccf4ccbf451d1435e3fbd13eb60878acbd09b77ff8` | get and put `ll`/`sc`; Clang target `mipsel-linux-gnu` |
...
and MIPS use `LLVM=1 LLVM_IAS=1`, query Clang with distinct target arguments,
and use LLVM objdump's `--disassemble-symbols` spelling; the nine GNU profiles retain
...
that `clone3` is not implemented; the accepted `-13` run observed no diagnostic.
...
PowerPC32, SuperH, Alpha, LoongArch64, little-endian MIPS32r2 and UML x86-64,
all under the exact SMP configurations
...
[eight-profile LL/SC capability audit](CONCURRENCY-C3-LLSC-PROGRESS-20260907.md)
...
with 20 conditional skips; all 23 focused IPC/refcount tests and all eleven
focused LL/SC capability tests pass.
